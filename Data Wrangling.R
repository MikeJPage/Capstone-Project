...
search_dates <- seq(as.Date("1990-01-01"), as.Date("2018-06-01"), by = "months")

# Set query parameters to be called

query <- sprintf("text:perfectionism AND language:en AND discoverDate:[%s TO %s]", search_dates, search_dates %m+% months(1))
...
perf_news %<>% mutate(text = stringi::stri_trans_general(perf_news$text, "latin-ascii"))
